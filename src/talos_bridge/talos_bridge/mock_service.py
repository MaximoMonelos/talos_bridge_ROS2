...
class PositionServiceMock(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(
            SetWheelPosition,
            'talos/set_wheel_position',
            self.set_position_callback
        )

        self.get_logger().info("Mock del Servicio SetWheelPosition iniciado y esperando comandos...")

    def set_position_callback(self, request, response):
            # 1. Leemos lo que nos pide el cliente (request)
            rueda = request.wheel_id
            posicion_deseada = request.position_deg # Asegurate de que esto coincida con tu .srv
            
            self.get_logger().info(f'¡Orden recibida! Moviendo rueda "{rueda}" a {posicion_deseada} grados.')
            ...
            response.message = f"Comando enviado y aceptado por la Raspberry Pi Pico para {rueda}"
            response.final_position_deg = posicion_deseada # Simulamos que llegó perfecto a la meta
